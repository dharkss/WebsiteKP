<x-layouts::app :title="__('Input Feed Mineral Dressing')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
            ...
            <form action="{{ route('mineral-dressing.input-feed.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @csrf
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Total Feed (Kg)</label>
                    <input type="number" step="0.01" name="total_feed" required placeholder="0.00"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-900 dark:border-gray-700 dark:text-white">
                </div>
